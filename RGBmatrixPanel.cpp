...
Tested with Arduino ATMega2560 and waterproof high brightness outdoor panels ordered from AliExpress:
https://goo.gl/ub1KkE
...
Where 2 is the number of panels. true = for double buffering: this reduces flickering but uses twice the amount of
memory.
...
void RGBmatrixPanel::init(
    uint8_t rows, uint8_t a, uint8_t b, uint8_t c, uint8_t clk, uint8_t lat, uint8_t oe, boolean dbuf, uint8_t pwidth)
{

    nMultiplexRows =
        2; // Used to double the rowlength for 1/4 scan panels, nRows will be 4 then for 16 led high display
...
RGBmatrixPanel::RGBmatrixPanel(
    uint8_t a, uint8_t b, uint8_t c, uint8_t clk, uint8_t lat, uint8_t oe, boolean dbuf, uint8_t pwidth)
    : Adafruit_GFX(32 * pwidth, 16)
{

    init(8, a, b, c, clk, lat, oe, dbuf, pwidth);
}

// Constructor for 32x32 panel:
RGBmatrixPanel::RGBmatrixPanel(
    uint8_t a, uint8_t b, uint8_t c, uint8_t d, uint8_t clk, uint8_t lat, uint8_t oe, boolean dbuf, uint8_t pwidth)
    : Adafruit_GFX(32 * pwidth, 32)
{

    init(16, a, b, c, clk, lat, oe, dbuf, pwidth);
...
{
    // RRRRrGGGGggBBBBb
    return ((r & 0xF) << 12) | ((r & 0x8) << 8) | ((g & 0xF) << 7) | ((g & 0xC) << 3) | ((b & 0xF) << 1) |
           ((b & 0x8) >> 3);
}
...
void RGBmatrixPanel::drawPixel(int16_t x, int16_t y, uint16_t c)
{
    ...
    if (y == 1 || y == 5 || y == 9 || y == 13)
        shift += 32 * 3 * nPanels * nMultiplexRows; // row 1
    if (y == 2 || y == 6 || y == 10 || y == 14)
        shift += 64 * 3 * nPanels * nMultiplexRows; // row 2
    if (y == 3 || y == 7 || y == 11 || y == 15)
        shift += 96 * 3 * nPanels * nMultiplexRows; // row 3
    ...
    if (y < (nRows * nMultiplexRows))
    {
        ...
        // Each base address has it's planes stored in 3 bytes, 32*nMultiplexRows*nPanels bytes apart (1 byte for 2
        // pixels: top/bottom). For 1/4 scan with 1 panel at relative addresses 0,64 and 128 Plane 0 is a tricky case --
        // its data is spread about, stored in least two bits not used by the other planes.
        ptr = ptr + (64 * nMultiplexRows * nPanels);
        *ptr &= ~B00000011; // Plane 0 R,G mask out in one op
        if (r & 1)
            *ptr |= B00000001; // Plane 0 R: 64 bytes ahead, bit 0
        if (g & 1)
            *ptr |= B00000010; // Plane 0 G: 64 bytes ahead, bit 1

        ptr = ptr - (32 * nMultiplexRows * nPanels);
        if (b & 1)
            *ptr |= B00000001; // Plane 0 B: 32 bytes ahead, bit 0
        else
            *ptr &= ~B00000001; // Plane 0 B unset; mask out
    ...
    else
    {
        ...
        *ptr &= ~B00000011; // Plane 0 G,B mask out in one op
        if (r & 1)
            ptr[32 * nMultiplexRows * nPanels] |= B00000010; // Plane 0 R: 32 bytes ahead, bit 1
        else
            ptr[32 * nMultiplexRows * nPanels] &= ~B00000010; // Plane 0 R unset; mask out
        if (g & 1)
...
void RGBmatrixPanel::updateDisplay(void)
{
    ...
    *latport |= latmask; // Latch data loaded during *prior* interrupt
    ...
    t = (nRows > 8) ? LOOPTIME
                    : (LOOPTIME *
                       2); // Multiplier of 2 for 32x32pixel panel? Could also apply for 1/4 since double the row length
    ...
    else if (plane == 1)
    {
        ...
        // lat now, so update the row address lines before we do that:
        if (row & 0x1)
            *addraport |= addramask;
        else
            *addraport &= ~addramask;
...
#define pew                                                                                                            \
    ...
    asm volatile("out %0,%1" ::"I"(_SFR_IO_ADDR(CLKPORT)), "r"(tick));                                                 \
    asm volatile("out %0,%1" ::"I"(_SFR_IO_ADDR(CLKPORT)), "r"(tock));
