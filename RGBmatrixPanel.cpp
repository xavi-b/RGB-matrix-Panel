...
void RGBmatrixPanel::init(uint8_t rows,
                          uint8_t a,
                          uint8_t b,
                          uint8_t c,
                          uint8_t clk,
...
void RGBmatrixPanel::begin(void)
{
    ...
    *addrcport &= ~addrcmask; // Low
    ...
    timer_isr_register(TIMER_GROUP_1, TIMER_0, IRQ_HANDLER, (void*)TIMER_0, ESP_INTR_FLAG_IRAM, NULL);
...
uint16_t RGBmatrixPanel::Color888(uint8_t r, uint8_t g, uint8_t b, boolean gflag)
{
    ...
    {                                       // Gamma-corrected color?
        ...
        return ((uint16_t)r << 12) | ((uint16_t)(r & 0x8) << 8) | // 4/4/4->5/6/5
               ((uint16_t)g << 7) | ((uint16_t)(g & 0xC) << 3) | (b << 1) | (b >> 3);
...
#define pew                                                                                                            \
    ...
    *outsetreg = clkmask;
    ...
    { // 920 ticks from TCNT1=0 (above) to end of function
        ...
        for (i = 0; i < WIDTH; i++)
        {
            DATAPORT = (ptr[i] << 6) | ((ptr[i + WIDTH] << 4) & 0x30) | ((ptr[i + WIDTH * 2] << 2) & 0x0C);
        ...
        {
            byte b = (ptr[i] << 6) | ((ptr[i + WIDTH] << 4) & 0x30) | ((ptr[i + WIDTH * 2] << 2) & 0x0C);
